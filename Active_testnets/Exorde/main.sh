...
mkdir /ExordeModuleCLI/log    
cat > /ExordeModuleCLI/run <<EOF 
#!/bin/bash
exec 2>&1
exec python3 Launcher.py -m $YOUR_WALLET_ADDRESS -l 2
EOF
chmod +x /ExordeModuleCLI/run
LOG=/var/log/Exorde
cat > /ExordeModuleCLI/log/run <<EOF 
#!/bin/bash
mkdir $LOG
exec svlogd -tt $LOG
EOF
chmod +x /ExordeModuleCLI/log/run
ln -s /ExordeModuleCLI /etc/service
